...
/// Una estructura personalizada, que ya es
/// serializable como `felt252`.
#[derive(Drop, Serde)]
struct MyData {
    a: felt252,
    b: felt252,
...
trait IContractL1<T> {
    ...
    /// Para enviar una estructura en la carga útil de un mensaje, solo necesitas asegurarte de que
    /// tu estructura sea serializable implementando los traits `Serde`.
...
mod WorkshopMensajesL2 {
    ...
    ///
    /// Solo las funciones que están anotadas con `#[l1_handler]` pueden
    /// recibir mensajes de L1, ya que el secuenciador ejecutará
    /// el código del contrato utilizando un tipo de transacción específico (`L1HandlerTransaction`)
    /// que solo puede ver los puntos finales anotados como tal.
    ///
    /// # Argumentos
    ///
    /// * `from_address` - El contrato L1 que envía el mensaje (WorkshopMensajesL1).
    /// * `value` - Valor esperado en la carga útil.
    ///
    /// En producción, siempre debes verificar si `from_address` es
    /// un contrato al que se le permite enviar mensajes, ya que cualquier contrato de L1
    /// puede enviar mensajes a cualquier contrato de L2 y viceversa.
    ///
    /// En este ejemplo, se espera que la carga útil sea un único valor `felt`.
    ...
    /// Maneja un mensaje recibido de L1.
    /// En este ejemplo, el manejador espera que los miembros de la estructura sean mayores que 0.
    ///
    /// # Argumentos
    ///
